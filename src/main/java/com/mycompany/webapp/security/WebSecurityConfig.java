...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
			...
			//URL 권한 설정. 권한 부족하면 403 error 발생
			http.authorizeRequests()
					.antMatchers("/security/admin/**").hasAuthority("ROLE_ADMIN") 		//"/security/admin/**" 앞에 admin까지만 맞으면 뒤에 뭐가 와도 상관없다.
					.antMatchers("/security/manager/**").hasAuthority("ROLE_MANAGER")
					.antMatchers("/security/user/**").authenticated() // 모든 인증된 사용자가 가능
		...
		protected void configure(AuthenticationManagerBuilder auth) throws Exception {
			...
			auth.jdbcAuthentication()
					...
					.passwordEncoder(passwordEncoder()); // default: DelegatingPasswordEncoder  - 데이터 베이스엔 (bcrypt) ~, (sha256) ~ 저장되어야 함.
			 */
